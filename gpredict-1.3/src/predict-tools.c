...
//	These two lines are from gtk_sat_data_init_sat
	jul_utc = Julian_Date_of_Epoch (sat->tle.epoch); // => tsince = 0.0
    sat->jul_epoch = jul_utc;

	if (qth)	// per gtk_sat_data_init_sat
	...
	sat->jul_utc = t?t:sat->jul_epoch;	/* time zero initialze to epoch */
	...
	/* orbit type (from gtk_sat_data_init_sat) */
 ...
 *
 */
void cdecl TraceLogThread(char *Name, int ForceIt, char *Format, ...);
#define CRITICAL_LOGGING FALSE
...
find_los (sat_t *sat, qth_t *qth, gdouble start, gdouble maxdt)
{
	...
	char *sat_name = sat->name?sat->name:sat->nickname?sat->nickname:sat->tle.sat_name;
    ...
    if ((sat->otype == ORBIT_TYPE_GEO) || 
        (sat->otype == ORBIT_TYPE_DECAYED) ||
        !has_aos (sat, qth)) {
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) Has NO AOS\n", sat_name);
        return 0.0;
    ...
    if (t < 0.01)
	{
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) find_aos() FAILED!\n", sat_name);
        return 0.0;
	}
    ...
    /* use upper time limit */
    if (maxdt > 0.0) {
		double lastEl = sat->el;

TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) maxdt Starting el=%lf\n", sat_name, (double) sat->el);
        /* coarse steps */
        while ((sat->el >= 1.0) && (t <= (start + maxdt))) {
        ...
        while ((lostime == 0.0) && (t <= (start + maxdt)))  {
            ...
            if (fabs(sat->el) < 0.005)
				...
				{	TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) Rising %lf->%lf\n", sat_name, (double) lastEl, (double) sat->el);
				}
			lastEl = sat->el;
        }
    }
...
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) unlimited Starting el=%lf\n", sat_name, (double) sat->el);
...
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_los(%s) Returning %lf from %lf Delta %lf Minutes\n", sat_name, (double) lostime, (double) start, (double) (lostime-start)*24.0*60.0);

    return lostime;
}

gdouble
find_max_el (sat_t *sat, qth_t *qth, gdouble start, gdouble end, int nesting, gdouble *tMax)
{
    gdouble t = start;
	double maxT = start;
    gdouble maxEl = 0.0;
	...
	if (end <= start)
	{
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) end(%lf) < start(%lf)\n", sat_name, (double) end, (double) start);
		return 0.0;
	}
    ...
    if ((sat->otype == ORBIT_TYPE_GEO) || 
        (sat->otype == ORBIT_TYPE_DECAYED) ||
        !has_aos (sat, qth)) {
TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) Has NO AOS\n", sat_name);
        return 0.0;
    }
	...
	if (!nesting)
	{
		if (sat->el < 0.0)
		{
	TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) find_aos() due to el(%lf)\n", sat_name, (double) sat->el);
			t = find_aos (sat, qth, start, end) + 0.001; // +1.5 min
		} else if (sat->el > 0.0)
		{	t = find_prev_aos (sat, qth, start);
		}

		/* invalid time (potentially returned by find_aos) */
		if (t < 0.01)
		{
	TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) find_aos/find_prev_aos() FAILED!\n", sat_name);
			return 0.0;
		}

		if (t > end)
		{
	TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) find_aos() AOS(%lf) > end(%lf)\n", sat_name, (double) t, (double) end);
			return 0.0;
		}
		start = t;	/* Move to AOS time */
	}
	...
	/* use upper time limit */
    {	double deltaT = (end-start)/10.0;	/* Step 10 times through pass */

TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) maxdt Starting el=%lf Stepping %lf to %lf by %lf\n", sat_name, (double) sat->el, start, end, deltaT);
TraceLogThread("MaxEl", TRUE, "find_max_el(%s) maxdt Starting el=%lf Stepping %lf to %lf by %lf seconds\n", sat_name, (double) sat->el, start, end, deltaT*24.0*60.0*60.0);
		for (t=start+deltaT; t<=end; t+=deltaT)
		{	predict_calc (sat, qth, t);
			if (sat->el > maxEl)
			{	maxSat = *sat;
				maxEl = sat->el;
...
//		lastEl = sat->el;
		...
		TraceLogThread(sat_name, CRITICAL_LOGGING, "find_max_el(%s) Found %lf @ %lf/%lf Minutes (Delta: %lf seconds)\n", sat_name, (double) maxEl, (double) (maxT-start)*24.0*60.0, (double) (end-start)*24.0*60.0, (double) deltaT*24.0*60.0*60.0);
		TraceLogThread("MaxEl", TRUE, "find_max_el(%s) Found %lf @ %lf/%lf Minutes (Delta: %lf seconds)\n", sat_name, (double) maxEl, (double) (maxT-start)*24.0*60.0, (double) (end-start)*24.0*60.0, (double) deltaT*24.0*60.0*60.0);

		if (nesting < 5						/* Max recursions */
		&& deltaT > 30.0/60.0/60.0/24.0)	/* 30 secondS */
		{	double newMax = find_max_el(sat, qth, max(start,maxT-deltaT), min(end,maxT+deltaT), nesting+1, tMax);
		TraceLogThread("MaxEl", TRUE, "find_max_el(%s) Refine %s by %lf from %lf to %lf\n", sat_name, newMax>maxEl?"Increased":newMax<maxEl?"DECREASED":"Equaled", (double) (newMax-maxEl), (double) maxEl, (double) newMax);
			if (newMax >= maxEl)
			{	maxEl = newMax;
				maxSat = *sat;
			} else if ((maxEl-newMax) >= 0.000001) TraceLogThread("Satellite", TRUE, "find_max_el(%s) Refine REDUCED by %lf from %lf to %lf\n", sat_name, (double) (maxEl-newMax), (double) maxEl, (double) newMax);
		} else if (tMax) *tMax = maxT;
	}
	*sat = maxSat;	/* Put it to the max */
    return maxEl;
}
